...
class ABC_B2B_Designer_AJAX {
    ...
    private function __construct() {
        ...
        add_action('wp_ajax_abc_b2b_save_design', [$this, 'save_design']);        // Draft workflow (team)
    ...
    public function save_design() {
                ...
                // Template must be accessible to current org unless bypass
                $org_id_required = (int) get_post_meta($template_id, '_abc_org_id', true);
                $org_id = abc_b2b_designer_current_user_org_id();
                if ($org_id_required && $org_id_required !== (int)$org_id && !$this->can_bypass()) {
                    wp_send_json_error(['message' => 'Template restricted.'], 403);
                }

                // Folder: /uploads/abc-b2b-designer/tmp/{token}/
                $token = abc_b2b_designer_token(24);
                $base_dir = abc_b2b_designer_upload_base_dir();
                $tmp_dir = trailingslashit($base_dir) . 'tmp/' . $token . '/';
                wp_mkdir_p($tmp_dir);

                $this->handle_file_saves($tmp_dir, $payload, $_POST['previews'] ?? [], $_POST['svgs'] ?? []);
    ...
    /**
     * Phase 0: SVG Sanitization policy (basic hard-block).
     * NOTE: For higher assurance, consider an SVG sanitizer library or disallow SVG storage entirely.
     */
    private function sanitize_svg(string $svg_content): string {
        // block common dangerous tags + javascript:
        if (preg_match('/<(script|object|embed|iframe|link|style|foreignObject)/i', $svg_content)) return '';
        if (preg_match('/on[a-z]+\s*=\s*(["\']).*?\1/i', $svg_content)) return '';
        if (preg_match('/javascript\s*:/i', $svg_content)) return '';
        if (strpos($svg_content, '<svg') === false || strpos($svg_content, '</svg>') === false) return '';
        return $svg_content;
    }

    /**
     * Phase 0: Standardized file handling for saving payload + previews + svgs.
     * $target_dir MUST be an absolute directory path ending with a trailing slash.
     */
    private function handle_file_saves(string $target_dir, string $payload, $previews, $svgs): array {
        // payload json
        $ok = @file_put_contents($target_dir . 'design.json', $payload);
        if ($ok === false) {
            wp_send_json_error(['message' => 'Unable to write design.json'], 500);
        }

        // Normalize arrays if passed as JSON strings (can happen depending on client)
        if (is_string($previews)) {
            $tmp = json_decode($previews, true);
            if (is_array($tmp)) $previews = $tmp;
        }
        if (is_string($svgs)) {
            $tmp = json_decode($svgs, true);
            if (is_array($tmp)) $svgs = $tmp;
        }

        $saved_surfaces = [];

        // PNG previews (strict allowlist)
        if (is_array($previews)) {
            foreach ($previews as $surface_key => $data_url) {
                ...
                // 8MB cap per preview
                if (strlen($decoded['bytes']) > 8 * 1024 * 1024) continue;

                $path = $target_dir . 'surface-' . $surface_key . '.png';
                $w = @file_put_contents($path, $decoded['bytes']);
                if ($w !== false) {
                    $saved_surfaces[] = $surface_key;
                }
            }
        }

        // SVGs (sanitized). Consider disabling in production.
        if (is_array($svgs)) {
            foreach ($svgs as $surface_key => $svg_str) {
                ...
                // 2MB cap
                if (strlen($svg_str) > 2 * 1024 * 1024) continue;
...
$tpl = get_post($template_id);
    ...
    public function save_draft() {
                ...
                // Folder: /uploads/abc-b2b-designer/drafts/{id}/current/
                $draft_dir = trailingslashit(abc_b2b_designer_upload_drafts_dir()) . (int)$draft_id . '/current/';
                wp_mkdir_p($draft_dir);

                $this->handle_file_saves($draft_dir, $payload, $_POST['previews'] ?? [], $_POST['svgs'] ?? []);
    ...
    public function set_employee_ready() {
                ...
                $ready = isset($_POST['ready']) ? (int) $_POST['ready'] : (isset($_POST['ready']) ? (int)$_POST['ready'] : 1);
                $ready = $ready ? 1 : 0;

                $this->assert_draft_access($draft_id);

                update_post_meta($draft_id, '_abc_employee_ready', $ready);
                if ($ready) {
                    ...
                    delete_post_meta($draft_id, '_abc_employee_ready_by');
                    delete_post_meta($draft_id, '_abc_employee_ready_at');
                }

                wp_send_json_success(['draft_id' => (int)$draft_id, 'ready' => $ready]);
    }
    ...
    public function set_admin_ready() {
                ...
                update_post_meta($draft_id, '_abc_admin_ready', $ready);
                if ($ready) {
                    ...
                    delete_post_meta($draft_id, '_abc_admin_ready_by');
                    delete_post_meta($draft_id, '_abc_admin_ready_at');
                }
    ...
    public function set_ready_override() {
                ...
                $override = null;
                if (isset($_POST['override'])) $override = (int) $_POST['override'];
                if ($override === null && isset($_POST['ready'])) $override = (int) $_POST['ready'];
                if ($override === null) $override = 0;
                $override = $override ? 1 : 0;

                // Only org admins or bypass can set override
                if (!abc_b2b_designer_current_user_is_org_admin() && !$this->can_bypass()) {
                    wp_send_json_error(['message' => 'Only Organization Admins can override approvals.'], 403);
                }
                ...
                wp_send_json_success(['draft_id' => (int)$draft_id, 'override' => $override]);
    }
