...
(function($){
  ...
  function wireUI(){
    ...
    $('#abc_employee_ready').on('click', function(){
      ...
      ajaxPost('abc_b2b_set_employee_ready', { draft_id: ABCDRAFTED.draft_id, ready: (state.draft && state.draft.employee_ready) ? 0 : 1 }).done(function(resp){
        if(!resp || !resp.success){
    ...
    $('#abc_admin_ready').on('click', function(){
      ...
      ajaxPost('abc_b2b_set_admin_ready', { draft_id: ABCDRAFTED.draft_id, ready: (state.draft && state.draft.admin_ready) ? 0 : 1 }).done(function(resp){
        if(!resp || !resp.success){
